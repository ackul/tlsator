...
class customRecord(dpkt.Packet):
  __hdr__ = (
      ('type', 'B', 0),
      ('version', 'H', 0),
      ('length', 'H', 0),
  ...
  def __init__(self,buf):
    self.type = struct.unpack('!B',buf[0])[0]
    self.version = struct.unpack('!H',buf[1:3])
    self.length = struct.unpack('!H',buf[3:5])
    self.data = 0
...
def TLSMultiFactoryAK(buf):
    ...
    ver = buf[1:3]
    lenRec = buf[3:5]
    #print "version No - %s" % binascii.hexlify(ver)
    lenRec = binascii.hexlify(lenRec)
    #print "Length of the record data -  %d" % int(lenRec,16)
    if ver in SSL3_VERSION_BYTES:
        try:
            if(int(lenRec,16) > len(buf)):
              msg = customRecord(buf)
              return msg,len(buf),1,int(lenRec,16)
            else:
              msg = TLSRecord(buf)
                #msgs.append(msg)
      ...
      raise SSL3Exception('Bad TLS version in buf: %r' % buf[3:5])
        #i += len(msg)
    #print "yoyoyoo"
    return msg, len(msg),0,int(lenRec,16)
